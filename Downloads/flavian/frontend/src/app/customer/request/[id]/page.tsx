...
export default function RequestDetailsPage({
    ...
    if (socket) {
      socket.on('order_status_updated', updatedRequest => {
        if (updatedRequest.id === params.id) {
          ...
          showToast(
            `Tracked order updated to ${updatedRequest.status}`,
            'info',
          );
        }
      });
  ...
  const fetchAdminContact = async () => {
    try {
      ...
      setAdminContact({ id: res.data.id, name: res.data.name });
    } catch (err) {
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1200, mx: 'auto' }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={7}>
          <Card sx={{ p: 3, mb: 3, borderRadius: 0 }}>
            <Box display='flex' justifyContent='space-between' mb={2}>
              ...
              <Chip
                ...
                color={request.status === 'CANCELLED' ? 'default' : 'primary'}
                sx={{ borderRadius: 1 }}
              />
            </Box>
            <Typography variant='body2' color='text.secondary'>
              Bags: {request.bagCount}
            </Typography>
          ...
          {request.status === 'PENDING' && (
            <Box>
              <Typography variant='h6' fontWeight='bold' mb={2}>
                Vendor Offers ({request.offers?.length || 0})
              </Typography>
                  ...
                  >
                    <Box>
                      <Typography variant='subtitle1' fontWeight='bold'>
                        {offer.vendor.vendorProfileVendor?.businessName ||
                          offer.vendor.name}
                      </Typography>
                      <Typography variant='body2' color='text.secondary'>
                        ETA: {offer.etaHours} hours
                      </Typography>
                    </Box>
                    <Box textAlign='right'>
                      <Typography
          ...
          {['ACCEPTED', 'PICKED_UP', 'WASHING', 'DELIVERING'].includes(
            request.status,
          ) && (
            <Box mt={3}>
              <Typography variant='h6' fontWeight='bold' mb={2}>
          ...
          >
            <Box
              sx={{
                ...
                height: '100%',
                backgroundImage:
                  'url("https://api.mapbox.com/styles/v1/mapbox/light-v10/static/-122.4194,37.7749,14,0/400x200?access_token=none")',
            ...
            >
              ...
              <Typography
                variant='caption'
                sx={{ bgcolor: 'white', px: 1, mt: 0.5, display: 'block' }}
              >
                Pickup Point
              </Typography>
            </Box>
          ...
          adminContact?.id ? (
            <>
              <Typography variant='subtitle2' mb={1} color='text.secondary'>
                You are chatting with admin support ({adminContact.name}).
              </Typography>
              <ChatBox requestId={request.id} receiverId={adminContact.id} />
            </>
          ) : (
            <Card
              sx={{
            ...
            >
              <Typography color='text.secondary'>
                {request.status === 'CANCELLED'
                  ? 'Order was cancelled.'
                  : 'Chat with admin support opens once an offer is accepted and an admin account is available.'}
              </Typography>
            </Card>
          )}
        </Grid>
